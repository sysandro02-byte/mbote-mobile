...
/**
 * Canonical authenticated WebSocket client for MBoté.
 * Chat/presence and Live signaling both use the production /ws gateway.
 */
object MboteSocketManager {
    ...
    @Volatile private var shouldReconnectChat = false
    ...
    fun connect(customUrl: String? = null, forceResetAttempts: Boolean = false) {
        ...
        if (_isConnected.value && currentToken == token && !forceResetAttempts) return

        shouldReconnectChat = true
        currentToken = token
        if (forceResetAttempts) reconnectAttempt = 0
        reconnectJob?.cancel()
        chatWebSocket?.close(1000, "reconnect")
        openChatSocket(target, token)
    }

    private fun openChatSocket(target: String, token: String) {
        _connectionState.value = if (reconnectAttempt == 0) SocketConnectionState.CONNECTING else SocketConnectionState.RECONNECTING
        _isConnected.value = false
        val request = Request.Builder().url(wsUrl(target)).build()
        chatWebSocket = httpClient.newWebSocket(request, object : WebSocketListener() {
            ...
            override fun onClosed(webSocket: WebSocket, code: Int, reason: String) {
                if (chatWebSocket === webSocket) chatWebSocket = null
                _isConnected.value = false
                if (shouldReconnectChat && MboteBackendConfig.authToken == token) scheduleReconnect(target, token)
                else _connectionState.value = SocketConnectionState.DISCONNECTED
            }

            override fun onFailure(webSocket: WebSocket, t: Throwable, response: Response?) {
                if (chatWebSocket === webSocket) chatWebSocket = null
                _isConnected.value = false
                Log.w(TAG, "WebSocket chat error: ${t.message}")
                if (shouldReconnectChat && MboteBackendConfig.authToken == token) scheduleReconnect(target, token)
                else _connectionState.value = SocketConnectionState.DISCONNECTED
            }
        })
    ...
    private fun scheduleReconnect(target: String, token: String) {
        if (reconnectJob?.isActive == true) return
        reconnectAttempt = (reconnectAttempt + 1).coerceAtMost(8)
        _connectionState.value = SocketConnectionState.RECONNECTING
        val waitMs = min(30_000L, 1_500L * reconnectAttempt)
        reconnectJob = scope.launch {
            delay(waitMs)
            if (shouldReconnectChat && MboteBackendConfig.authToken == token) openChatSocket(target, token)
        }
    }
    ...
    fun disconnect() {
        ...
        reconnectJob?.cancel()
        reconnectJob = null
        chatWebSocket?.close(1000, "logout")
        chatWebSocket = null
        currentToken = null
        reconnectAttempt = 0
    ...
    private fun handleMessage(data: JSONObject) {
        ...
        _incomingMessages.tryEmit(
            SocketChatMessage(
                ...
                senderName = data.optString("senderName", data.optString("sender_name", "Utilisateur MBoté")),
                senderAvatar = data.optString("senderAvatar", data.optString("sender_avatar")).ifBlank { null },
                text = data.optString("text", data.optString("content")),
                isMedia = data.optString("mediaType", "NONE") != "NONE",
                mediaUrl = data.optString("mediaUrl", data.optString("media_url")).ifBlank { null }
            )
        )
    }

    private fun handleTyping(data: JSONObject) {
        val chatId = data.optString("chatId", data.optString("chat_id"))
        if (chatId.isBlank()) return
        onRemotePartnerTypingReceived(
            chatId = chatId,
            userName = data.optString("userName", "Interlocuteur"),
            isTyping = data.optBoolean("isTyping", true)
        )
    }

    fun sendRawEvent(jsonPayload: String): Boolean = chatWebSocket?.send(jsonPayload) ?: false

    /**
     * Chat messages must be persisted through the REST API. The WebSocket gateway
     * broadcasts the persisted server message to the other participants.
     */
    fun sendChatMessage(chatId: String, senderName: String, text: String, mediaUrl: String? = null) {
        Log.w(TAG, "sendChatMessage ignored: persist messages through MboteApiService before realtime broadcast")
    }

    fun sendUserTypingState(chatId: String, isTyping: Boolean, userName: String = "Vous") {
        if (!_isConnected.value || chatId.isBlank()) return
        chatWebSocket?.send(
            JSONObject()
    ...
    fun connectLiveWebSocket(streamId: String) {
        val token = MboteBackendConfig.authToken?.trim().orEmpty()
        if (token.isBlank() || streamId.isBlank()) return
        liveWebSocket?.close(1000, "reconnect")
        val request = Request.Builder().url(wsUrl(BuildConfig.VITE_SOCKET_URL)).build()
        liveWebSocket = httpClient.newWebSocket(request, object : WebSocketListener() {
            override fun onOpen(webSocket: WebSocket, response: Response) {
                webSocket.send(JSONObject().put("type", "AUTH").put("token", token).toString())
            }
            ...
            override fun onMessage(webSocket: WebSocket, text: String) {
                ...
                when (data.optString("type")) {
                    "AUTH_OK" -> {
                        _liveSocketIdentity.value = data.optString("userId").ifBlank { null }
                        webSocket.send(JSONObject().put("type", "LIVE_JOIN").put("streamId", streamId).toString())
                    }
            ...
            override fun onFailure(webSocket: WebSocket, t: Throwable, response: Response?) {
                Log.w(TAG, "WebSocket Live error: ${t.message}")
            }
        })
    }
    ...
    fun sendLiveComment(streamId: String, senderName: String, text: String, badgeType: String? = null) {
        liveWebSocket?.send(
            JSONObject().put("type", "LIVE_COMMENT").put("streamId", streamId)
                .put("text", text).put("badgeType", badgeType).toString()
        )
    }

    fun sendLiveReaction(streamId: String, senderName: String, emoji: String) {
        liveWebSocket?.send(
            JSONObject().put("type", "LIVE_REACTION").put("streamId", streamId).put("emoji", emoji).toString()
        )
    }

    fun sendLiveGift(streamId: String, senderName: String, giftId: String, giftName: String, emoji: String, valueFcfa: Long) {
        liveWebSocket?.send(
            JSONObject().put("type", "LIVE_GIFT").put("streamId", streamId)

...
const LEGACY_UUID_TYPES = new Set([...INTEGER_TYPES, ...TEXT_TYPES]);
...
async function getForeignKeysTo(db, targetTable) {
  const result = await db.query(`
    SELECT DISTINCT tc.table_name, kcu.column_name, tc.constraint_name, rc.delete_rule, rc.update_rule
    ...
    WHERE tc.constraint_type = 'FOREIGN KEY' AND tc.table_schema = 'public'
      AND ccu.table_schema = 'public' AND ccu.table_name = $1 AND ccu.column_name = 'id'
  `, [targetTable]);
  return result.rows;
}
...
function safeRule(rule) {
  return ['CASCADE', 'SET NULL', 'SET DEFAULT', 'RESTRICT', 'NO ACTION'].includes(rule) ? rule : 'NO ACTION';
}
...
async function migrateLegacyIntegerEntity(db, table) {
  ...
  if (!LEGACY_UUID_TYPES.has(idType.udt_name)) throw new Error(`Type ${table}.id historique non pris en charge: ${idType.udt_name}`);
  ...
  try {
    for (const ref of refs) {
      ...
      if (!refType) continue;
      if (refType.udt_name !== 'uuid' && !LEGACY_UUID_TYPES.has(refType.udt_name)) {
    ...
    for (const ref of refs) {
      ...
      await db.query(`ALTER TABLE ${q(ref.table_name)} ADD CONSTRAINT ${q(ref.constraint_name)} FOREIGN KEY (${q(ref.column_name)}) REFERENCES ${q(table)}(id) ON UPDATE ${safeRule(ref.update_rule)} ON DELETE ${safeRule(ref.delete_rule)}`);
    }
...
async function migrateLegacyIntegerUsers(db) {
  ...
  if (!INTEGER_TYPES.has(usersId.udt_name)) throw new Error(`Type users.id historique non pris en charge: ${usersId.udt_name}`);

  console.log(`[mbote-db] Ancien schéma détecté: users.id=${usersId.udt_name}. Migration UUID...`);
  await db.query('BEGIN');
  try {
    await db.query('CREATE EXTENSION IF NOT EXISTS "uuid-ossp"');
    await db.query(`CREATE TABLE IF NOT EXISTS legacy_user_uuid_map (legacy_id BIGINT PRIMARY KEY, user_uuid UUID UNIQUE NOT NULL DEFAULT uuid_generate_v4(), migrated_at TIMESTAMPTZ NOT NULL DEFAULT NOW())`);
    ...
    for (const ref of refs) {
      ...
      await db.query(`ALTER TABLE ${q(ref.table_name)} ADD CONSTRAINT ${q(ref.constraint_name)} FOREIGN KEY (${q(ref.column_name)}) REFERENCES users(id) ON UPDATE ${safeRule(ref.update_rule)} ON DELETE ${safeRule(ref.delete_rule)}`);
    }
...
function uuidPrimaryKeyTablesFromSchema(schema) {
  const tables = [];
  const createTable = /CREATE\s+TABLE\s+IF\s+NOT\s+EXISTS\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*\(([\s\S]*?)\);/gi;
  let match;
  while ((match = createTable.exec(schema)) !== null) {
    if (/\bid\s+UUID\s+PRIMARY\s+KEY\b/i.test(match[2])) tables.push(match[1]);
  }
...
async function main() {
  ...
  try {
    ...
    const check = await db.query(`SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name = ANY($1::text[])`, [required]);
